...
Then, we'll run `python -m train.train_all train_variants --output_dir train_all_variants`. This will run PPO on the mug geometries and save training statistics to `train_all_variants/`. 
...
From there, we'll run `python -m labeling.label_assets_from_scores output.csv train_variants`. This will label the mugs we just PPO'd. 
...
python -m run_experiments.run_curriculum_experiments \
  ...
  --output_dir experiment_runs_second_seed/curriculum_vs_baselines \
...
This will save training statistics as well as evaluation statistics per method per seed which we can analyze to determine sample efficiency!

To evaluate how our method performs on OOD geometries against the baselines, we'll first generate a set of ood distributions (we need to change the ranges in cup.py to be OOD) in the same way as we did in the Generating Mugs section and we'll save them to `ood_variants\`. Then, we want to extract the saved final models from our previous experiment's output directory and then set them up in this hierarchy:
